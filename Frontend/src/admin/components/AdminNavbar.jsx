    ...
    <header className="admin-navbar sticky top-0 z-20 border-b border-[#e6d8c2] bg-[#fbf5ea]/95 px-4 py-4 sm:px-6 lg:px-8">
      <div className="mx-auto flex max-w-7xl flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex items-center justify-between gap-4">
          <Link className="flex items-center gap-3 font-black text-[#24211f]" to="/admin/dashboard">
        ...
        <nav className="admin-nav-scroll flex gap-2 overflow-x-auto pb-1">
          {items.map((item) => (
            <NavLink
              className={({ isActive }) =>
                `admin-nav-link rounded-full px-4 py-2 text-sm font-bold ${isActive ? "admin-nav-link-active" : "text-[#70665c] hover:bg-[#fffaf1]"
                }`
              }
              key={item.href}
              to={item.href}
